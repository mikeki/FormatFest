class UsersController < ApplicationController
  ...
  def index
    if params[:usuario] == "3"
      ...
      @users = User.paginate(:page => params[:page],:per_page=>10, :order => "id asc", :conditions => ['email LIKE ? AND admin = \'1\' or colaborator = \'1\'', "%#{params[:search]}%"])
    elsif params[:usuario] == "2"
      @title = "Listado de Administradores"
      @users = User.paginate(:page => params[:page],:per_page=>10, :order => "id asc", :conditions => ['email LIKE ? AND admin = \'1\'', "%#{params[:search]}%"])
    elsif params[:usuario] == "1"
      @title = "Listado de Colaboradores"
      @users = User.paginate(:page => params[:page],:per_page=>10, :order => "id asc", :conditions => ['email LIKE ? AND colaborator = \'1\'', "%#{params[:search]}%"])
    elsif params[:usuario] == "0"
      @title = "Listado de Clientes"
      @users = User.paginate(:page => params[:page],:per_page=>10, :order => "id asc", :conditions => ['email LIKE ? AND admin = \'0\' AND colaborator = \'0\'', "%#{params[:search]}%"])
    else 
      @title = "Listado de todos los Usuarios"
      @users = User.paginate(:page => params[:page],:per_page=>10, :order => "id asc", :conditions => ['email LIKE ?', "%#{params[:search]}%"])
    end
  end
